...
function SceneOne() {
  ...
  return <div className="space-y-4">
    <StageShell label={`Urban studio at blue hour. A detailed filmmaker in a charcoal jacket is ${description}. His left-facing profile, nose, and eye line point toward the open street. Thirds grid is ${grid ? "visible" : "hidden"}.`}>
      ...
      <div className="absolute bottom-[4%] w-[24%] motion-safe:transition-[left,filter] motion-safe:duration-700 motion-safe:ease-out" style={{ left: `calc(${x}% - 12%)`, filter: placement === "left edge" ? "brightness(.88)" : "brightness(1)" }}><CinematicImage src={characterLeft} alt="Adult Black filmmaker in left-facing profile, wearing a charcoal jacket with gold trim and a camera bag." className="h-auto object-contain drop-shadow-2xl"/><div className="absolute bottom-[1%] left-[8%] h-[5%] w-[85%] rounded-full bg-background/70 blur-md" aria-hidden="true"/></div>
